...
import { TlFilers } from './types'

export type TlFilterFn = (posts: AppBskyFeedDefs.FeedViewPost[]) => AppBskyFeedDefs.FeedViewPost[]

export function tlFiltersToFn({ reply, repost }: TlFilers, myDid: string): TlFilterFn {
...
function excludeRepliesToNoFollowing(posts: AppBskyFeedDefs.FeedViewPost[], myDid: string) {
  return posts.filter((post) => {
    // FIXME: check if the root is also a following account.
    if (post.reply?.parent.author.viewer) {
      return !!post.reply.parent.author.viewer.following || post.reply.parent.author.did === myDid
    }
    return true
  })
}
